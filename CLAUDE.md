...
- Patient/device ID is parsed from the filename prefix that precedes the `YYYYMMDD_HHMMSS` pattern, via `patientIdFromFilename` (e.g. `M1709234_20260701_085737.csv` → `M1709234`); falls back to the label "患者ID不明" if no prefix is present. Compare/overall views show a patient-selector tab row (only when 2+ distinct patients are loaded) above the weekday tab row — selecting a patient first narrows `presentWeekdays` to that patient's own days, then the weekday tab narrows further. Both filters combine into `sheetResults`, which drives the comparison table/charts, the overall stats, the AI-prompt generator, and the PDF print header (`scopeLabel` in `BVPRRAnalyzerApp`). The single-view file list also tags each entry with `[patientId]` whenever multiple patients are loaded.
